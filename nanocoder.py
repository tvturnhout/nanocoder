...
from pathlib import PathVERSION = 38
TAGS = {"edit": "edit", "find": "find", "replace": "replace", "request": "request_files", "drop": "drop_files", "commit": "commit_message", "shell": "shell_command", "create": "create"}
SYSTEM_PROMPT = f'You are a coding expert. Answer any questions the user might have. If the user asks you to modify code, use this XML format:\n[{TAGS["edit"]} path="file.py"]\n[{TAGS["find"]}]lines to find[/{TAGS["find"]}]\n[{TAGS["replace"]}]new code[/{TAGS["replace"]}]\n[/{TAGS["edit"]}]\nThe [{TAGS["find"]}] text is replaced literally, so it must match exactly. Keep it short - only enough lines to be unambiguous. Split large changes into multiple small edits.\nTo delete, leave [{TAGS["replace"]}] empty. To create a new file: [{TAGS["create"]} path="new_file.py"]file content[/{TAGS["create"]}].\nTo request files (one path per line):\n[{TAGS["request"]}]\npath/file1.py\npath/file2.py\n[/{TAGS["request"]}]\nTo drop files from context (one path per line):\n[{TAGS["drop"]}]\npath/file.py\n[/{TAGS["drop"]}]\nTo run a shell command: [{TAGS["shell"]}]echo hi[/{TAGS["shell"]}]. The tool will ask the user to approve (y/n). After running, the shell output will be returned truncated (first 10 lines, then a TRUNCATED marker, then the last 40 lines; full output if <= 50 lines).\nWhen making edits provide a [{TAGS["commit"]}]...[/{TAGS["commit"]}].'.replace('[', '<').replace(']', '>')
...
def get_map(root, max_files=100):
    ...
    total_tok, lines_out = 0, []
    fmt = lambda t: f"{t//1000}k" if t >= 1000 else str(t)
    for f in files:
        p = Path(root, f)
        ...
        tok = sz // 4
        total_tok += tok
        if p.suffix.lower() in BINARY_EXT: lines_out.append(f"{f} [binary, {sz}B]"); continue
        defs = ""
        if f.endswith('.py'):
            try: defs = ": " + ", ".join(n.name for n in ast.parse(p.read_text()).body if isinstance(n, (ast.FunctionDef, ast.ClassDef)))[:80]
            except: pass
        lines_out.append(f"{f} [~{fmt(tok)} tok]{defs}")
    more = f" (+{len(files)-max_files} more)" if len((run(f"git -C {root} ls-files") or "").splitlines()) > max_files else ""
    return f"({len(files)} files, ~{fmt(total_tok)} tok){more}\n" + "\n".join(lines_out)
...
def main():
    ...
    autopilot = "--no-autopilot" not in sys.argv
    ...
    while True:
        ...
        while True:
            ...
            file_sizes = sorted([(f, len(safe_read(f))//4) for f in context_files if Path(repo_root,f).exists()], key=lambda x: -x[1])
            tok_files = sum(t for _,t in file_sizes)
            context_warning = ""
            if tok_files > 20000:
                context_warning = f"\n### Context Warning\nFiles in context are using ~{tok_files//1000}k tokens. Consider dropping files you no longer need:\n" + "\n".join(f"- {f} [~{t//1000}k tok]" for f,t in file_sizes[:5])
            system_prompt = SYSTEM_PROMPT
            messages = [{"role": "system", "content": system_prompt}, {"role": "system", "content": f"System summary: {json.dumps(system_summary(), separators=(',',':'))}"}] + history + [{"role": "user", "content": f"{context}{context_warning}\nRequest: {request}"}]
            ...
            for cmd in [s.strip() for s in shell_commands]:
                ...
                if answer == "y":
                    try: output_lines, exit_code = run_shell_interactive(cmd); shell_results.append(f"$ {cmd}\nexit={exit_code}\n" + "\n".join(truncate(output_lines)))
                    except Exception as err: shell_results.append(f"$ {cmd}\nerror: {err}")
                else: shell_results.append(f"$ {cmd}\nDENIED by user.")
            if autopilot and (added_files or shell_results):
                parts = []
                if added_files: parts.append(f"Added files: {', '.join(added_files)}")
                if shell_results: parts.append("Shell results:\n" + "\n\n".join(shell_results))
                request = "\n\n".join(parts) + "\nPlease continue."
                continue
            break
